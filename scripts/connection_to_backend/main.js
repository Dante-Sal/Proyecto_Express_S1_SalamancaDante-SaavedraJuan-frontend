document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (meResponse.status === 401) window.location.href = window.location.origin + meResult.redirect;
    
    if (meResult.ok) {
        const data = meResult.document;
        if (data.role === 'admin') window.location.href = window.location.origin + data.redirect.admin;
